...
/*Вторая реализация только с числом треков ^ */

SELECT COUNT(id_track) FROM tracks AS a 
JOIN albums AS b ON a.id_album=b.id_album
WHERE b.year_album BETWEEN 2019 AND 2020;
...
WHERE name_performer NOT IN ( /* Где имя исполнителя не входит в вложенную выборку */
    SELECT name_performer FROM performers AS a
    JOIN perf_alb AS b ON a.id_performer=b.id_performer 
    JOIN albums AS c ON b.id_album=c.id_album 
    WHERE c.year_album = 2020
);
